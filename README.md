...
Validator can be a composed set of validators built-in or custom validator using `PhoneValidator.compose`, see example
below.

Note that when composing validators, the sorting is important as the error message displayed is the first validator
failing.
...
* **CountrySelectorNavigator.draggableBottomSheet**
  Open a modal bottom sheet expanded horizontally which may be dragged from a minimum to a maximum of current available
  height.
...
selectorNavigator
:
CustomCountrySelectorNavigator
(
)
,
// ...
...
This package uses the `country_selector` package under the hood, which exports a method for dynamic localization
...
If one of the language you target is not supported you can submit a pull request in country_selector and
phone_form_field repositories.

# Overwriting or adding custom flags

Some users have expressed their need to change some flags due to political reasons, or stylistic reasons. You might also
wish to add your own flags. To do so refer to this issue: https://github.com/cedvdb/phone_form_field/issues/222